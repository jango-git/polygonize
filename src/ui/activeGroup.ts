...
// the root). Returns null if the selection no longer resolves to a live group.
export function getActiveGroup(): GroupUUID | null {
  const sel = getSelection();
  if (!sel) return null;
  const stack = getStack();
  if (sel.type === "group") {
    return stack.some((e) => e.type === "group" && e.group.uuid === sel.uuid) ? sel.uuid : null;
  }
